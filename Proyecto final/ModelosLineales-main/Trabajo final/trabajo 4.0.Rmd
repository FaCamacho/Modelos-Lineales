...
matriz_correlacion <- cor(cuanti) 
 ...
 p =ggcorrplot::ggcorrplot(
   ...
   labs(title= "Correlacion entre variables" ,caption = "Grafico 1)")
...
ggplot(qq, aes(x = empiricos, y=..density..)) +
  ...
  labs(caption = "Gráfico 6)",title = "Residuos externamente estudentizados")
...
Finalmente por principio de parsimonia, nos quedamos con un modelo en el cual tiene una transformacion logaritmica tanto en la variable peso como en la variable predictora Longitud1. Dicho modelo supera los diagnosticos de homoscedasticidad y normalidad. Como tambien posee menor  AIC, BIC y el mayor  R^2 ajustado.
...
Existen varios metodos para seleccionar entre distintos modelos, en la siguiente tabla se reportan el coeficiente$R^2$ ajustado, como tambien se reportan los criterios de informacion AIC y BIC 

```{r}


 bic1 = BIC(modelo)
bic2= BIC(modelo_ajustado2)
bic3 = BIC(mod3)
aic1 = AIC(modelo)
aic2=AIC(modelo_ajustado2)
aic3= AIC(mod3)
