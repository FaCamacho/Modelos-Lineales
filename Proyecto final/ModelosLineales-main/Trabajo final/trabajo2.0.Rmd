...
datos2 = biometria_peces

datos2 = datos2 %>%  mutate(Peso_gr = as.numeric(Peso_gr),Especie = as.factor(Especie) )
...
datos2 <- datos2 %>% filter(.,Peso_gr > 0)

datos_especie <- datos2 %>%
  group_by(Especie) %>%
  summarise(media = round(mean(Peso_gr),2),
            desvio = round(sd(Peso_gr),2))



mod1 <- lm(Peso_gr ~ Especie, data = datos2) #Modelo con "Peso ~ Especie"

mod0 <- lm(Peso_gr ~ 1, data = datos2) #Modelo con "Peso ~ 1"
...
#p-value: 4.058e-13 de la prueba de significación global
# Rechazamos H0) y afirmamos que la especie es relevante para determinar el peso de un pez


summary(lm((Peso_gr) ~ Especie + 0, data = datos2))
...
mod1 <- lm(log(Peso_gr) ~ log(Longitud1) + Especie, data = datos2)

###Supuestos de linealidad

#Homosedasticidad

datos2$r_i <- residuals(mod1)  

# residuos

datos2$t_i <- rstudent(mod1) #studentizados EXTERNAMENTE 

datos2$pred <- fitted(mod1)
...
shapiro.test(datos2$t_i) #Rechazo H0)
tseries::jarque.bera.test(datos2$t_i) #Rechazo H0)
ks.test(datos2$t_i, 'pnorm') #No Rechazo H0)
...
with(datos2,cor(Peso_gr,Longitud1))
...
mod2 <- lm(log(Peso_gr)~ log(Longitud1) + Especie + log(Longitud1):Especie, data = datos2)
...
anova(lm(log(Longitud1) ~ Especie, data = datos2))
...
ggplot(datos2, aes(x = log(Peso_gr), y = log(Longitud1), col = Especie)) +
  geom_point() +
  xlab('Peso') + 
  ylab('Longitud') +
  theme_bw()+
...
mod3 <- lm(log(Peso_gr) ~ log(Ancho_cm) + Especie  , data = datos2)

###Supuestos de modelos lineales

#Linealidad

with(datos2,cor(Peso_gr,Ancho_cm))

plot(log(datos2$Peso_gr), log(datos2$Ancho_cm))

plot(lm(lm(log(Peso_gr) ~ log(Ancho_cm)  , data = datos2)))

#Homosedasticidad

datos2$r_i <- residuals(mod3)  

# residuos

datos2$t_i <- rstudent(mod3) #studentizados EXTERNAMENTE 

datos2$pred <- fitted(mod3)
...
# Supuesto igualdad de pendientes
mod4 <- lm(Peso_gr~ Ancho_cm + Especie + Ancho_cm:Especie, data = datos2)
...
# Supuesto la covariable NO afecta la media de cada especie
anova(lm(Ancho_cm ~ Especie, data = datos2))
...
ggplot(datos2, aes(x = Peso_gr, y = Ancho_cm, col = Especie)) +
  geom_point() +
  xlab('Peso') + 
  ylab('Ancho') +
  theme_bw()+
...
grid.arrange(ancho,alto,longitud)
