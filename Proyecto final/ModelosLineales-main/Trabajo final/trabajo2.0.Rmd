...
```{r}
 #Ir agregando de forma paulatina cada variable
...
breusch_pagan(modelo_ajustado2) #No Rechazo H0) con p-valor de 0.8
...
datos_especie <- datos %>%
  ...
  summarise(media = round(mean(Peso_gr),2),
            ...
            min=round(min(Peso_gr),2),
            max=round(max(Peso_gr),2))
...
#Con un p-valor de 3.239e-13 la especie es significativa para el modelo
